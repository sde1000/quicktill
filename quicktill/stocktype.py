...
class remove_duplicate_stocktypes(command):
    """Remove duplicate stocktypes from the database

    Where duplicates are found, the one with the highest price is
    kept.
    """
    command = "remove-duplicate-stocktypes"
    help = "remove duplicate stocktypes from the database"

    @staticmethod
    def run(args):
        # All discovered stocktypes, indexed by (manufacturer, name,
        # abv, dept_id, unit_id), value is list of StockType
        with td.orm_session():
            st = {}
            stocktypes = td.s.query(StockType).order_by(StockType.id).all()
            for t in stocktypes:
                key = (t.manufacturer.strip(), t.name.strip(),
                       t.abv, t.dept_id, t.unit_id)
                l = st.setdefault(key, [])
                l.append(t)
            for key, stocktypes in st.items():
                if len(stocktypes) > 1:
                    print("{} has {} stocktypes".format(key, len(stocktypes)))
                    saleprice = max(t.saleprice for t in stocktypes)
                    pricechanged = max(t.pricechanged or datetime.datetime(
                        1970, 1, 1, 0, 0, 0) for t in stocktypes)
                    keep = stocktypes[0]
                    keep.saleprice = saleprice
                    keep.pricechanged = pricechanged
                    for t in stocktypes[1:]:
                        for sl in list(t.stocklines):
                            sl.stocktype = keep
                        for s in list(t.items):
                            s.stocktype = keep
                        td.s.delete(t)
